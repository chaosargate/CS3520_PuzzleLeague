...
Gem* Grid::getGem(int x, int y) const noexcept {
    Gem *gem = grid_[x][y];
    return gem;
}
...
void Grid::setIsMoving(bool isMoving) noexcept {
    isMoving_ = isMoving;
}
/**
                    ...
                    if (grid_[i][j]->getMatch()) {
                        for (int n = i - 1; n > 0; n--) {
                            if (!grid_[n][j]->getMatch()) {
                                cout << "Before: " << grid_[n][j]->getCol()
                                        << grid_[n][j]->getRow() << endl;
                                cout << "Before: " << grid_[i][j]->getCol()
                                        << grid_[i][j]->getRow() << endl;
                                swap(&(grid_[n][j]), &(grid_[i][j]));

                                cout << "After: " << grid_[n][j]->getCol()
                                        << grid_[n][j]->getRow() << endl;
                                cout << "After: " << grid_[i][j]->getCol()
                                        << grid_[i][j]->getRow() << endl;
 ...
 * decrease their transparency by 10 if it is greater than 10.
 */
